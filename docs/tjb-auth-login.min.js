import WebComponent from"https://tjb-webcomponents.github.io/tjb-webcomponent/tjb-wc.min.js";import html from"https://tjb-webcomponents.github.io/html-template-string/html-template-string.js";import{bounce}from"https://tjb-webcomponents.github.io/tjb-gfx/tjb-gfx.min.js";import"https://tjb-webcomponents.github.io/tjb-input/tjb-input.min.js";import"https://tjb-webcomponents.github.io/tjb-statusbar/tjb-statusbar.min.js";import"https://tjb-webcomponents.github.io/tjb-notify/tjb-notify.min.js";class tjbAuthLogin extends WebComponent(){// Styles
///////////////////////////////////////////////////////////
CSS(){return html`
      <style>
        :host {
...
HTML(){this.statusbar=html`
    ...
    `}// Attribute Handling
////////////////////////////////////////////////////////////
static get observedAttributes(){return["postbody","posturl"]}// Logic
////////////////////////////////////////////////////////////
loginHandler(event){event.preventDefault();if(!this.checkValidity())return false;this.statusbar.status="loading";const postbody=this.postbody&&this.postbody.replace(/\'/g,"\"");const body=Object.assign({},{email:this.emailInput.value,password:this.passwordInput.value},JSON.parse(postbody||"{}"));this.dispatchEvent("login",body);if(!this.posturl)return false;return fetch(this.posturl,{method:"POST",redirect:"follow",credentials:"include",headers:new Headers({"Content-Type":"application/json"}),body:JSON.stringify(body)}).then(resp=>resp.json()).then(resp=>{if(resp.error)throw resp;return this._loginSuccess(resp)}).catch(resp=>this._loginError(resp))}_loginSuccess(resp){this.success().then(this.dispatchEvent.bind(this,"success",resp))}_loginError(resp){this.dispatchEvent("error",resp);this.error()}success(){return bounce(this.domNode)}error(){return this.errorHandler()}checkValidity(){return[this.emailInput.checkValidity(),this.passwordInput.checkValidity()].every(check=>check)}openHandler(event,target){event.preventDefault();bounce(event.target).then(this._location.bind(this,event.target.href,target))}_location(href,target){this.dispatchEvent("redirect",{href,target})}errorHandler(){this.writeMessageError=this.writeMessageError.bind(this);this.statusbar.status="alert";this.domNode.addEventListener("animationend",this.writeMessageError);this.domNode.classList.remove("alert");setTimeout(()=>this.domNode.classList.add("alert"),100)}writeMessageError(){this.domNode.removeEventListener("animationend",this.writeMessageError);this.messageNode.error=html`
      <ul>
        <li>
          <a
    ...
    `;this.emailInput.showMessage("error");this.passwordInput.showMessage("error")}}customElements.define("tjb-auth-login",tjbAuthLogin);
